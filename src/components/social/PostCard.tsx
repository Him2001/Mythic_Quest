...
const PostCard: React.FC<PostCardProps> = ({ post, currentUser, onLike, onComment }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg border border-amber-100 overflow-hidden fantasy-card">
      ...
      <div className="p-4 border-b border-gray-100">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <div>
              <div className="flex items-center">
                <h3 className="font-cinzel font-bold text-gray-800">{post.userName}</h3>
                <Badge color="accent" size="sm" className="ml-2">
                  Level {post.userLevel}
                </Badge>
              </div>
              <p className="text-sm text-gray-500 font-merriweather">{timeAgo}</p>
            </div>
          </div>
      ...
      {/* Post Content */}
      <div className="p-4">
        <p className="text-gray-800 font-merriweather mb-4 leading-relaxed">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center space-x-4">
            <button
              onClick={handleLike}
              disabled={isLiking}
              className={`flex items-center space-x-2 transition-all duration-200 px-2 py-1 rounded-lg hover:bg-gray-50 ${
                isLiked ? 'text-red-500' : 'text-gray-500 hover:text-red-500'
              } ${isLiking ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              <Heart size={20} className={`${isLiked ? 'fill-current' : ''} ${isLiking ? 'animate-pulse' : ''}`} />
              <span className="font-merriweather text-sm font-medium">{post.likes.length}</span>
            </button>
            
            <button
              onClick={() => setShowComments(!showComments)}
              className="flex items-center space-x-2 text-gray-500 hover:text-blue-500 transition-colors duration-200 px-2 py-1 rounded-lg hover:bg-gray-50"
            >
              <MessageCircle size={20} />
              <span className="font-merriweather text-sm font-medium">{post.comments.length}</span>
            </button>
            
            <button className="flex items-center space-x-2 text-gray-500 hover:text-green-500 transition-colors duration-200 px-2 py-1 rounded-lg hover:bg-gray-50">
              <Share2 size={20} />
            </button>
          </div>
        </div>
                  ...
                  <div key={comment.id} className="flex items-start space-x-3">
                    ...
                    <div className="flex-1 bg-gray-50 rounded-lg p-3">
                      <div className="flex items-center mb-1">
                        <span className="font-cinzel font-bold text-sm text-gray-800">
                          {comment.userName}
                        </span>
            ...
            <form onSubmit={handleSubmitComment} className="flex items-start space-x-3">
              ...
              <div className="flex-1 flex space-x-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 border border-gray-200 rounded-full focus:outline-none focus:border-amber-500 font-merriweather text-sm"
